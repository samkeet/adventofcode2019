from part1 import getManhattanDistance
import sys

def constructPath(line):
    path = {}
    currX = 0
    currY = 0
    steps = 0
    ...
    for move in line:
        ...
        while distance:
            ...
            distance -= 1
            steps += 1
            if (currX, currY) not in path:
                path[(currX, currY)] = steps
    return path

def findIntersections(path_one, path_two):
    '''Returns a list of common points
    '''

    common_points = {}
    for p1 in path_one:
        if p1 in path_two:
            common_points[p1] = path_one[p1] + path_two[p1]

    return common_points

def getBestPoints(common_points):

    best_point_dist = min(common_points.values())
    best_points = list(filter(lambda x: (common_points[x] == best_point_dist), common_points.keys()))

    if len(best_points) > 1:
        print("Got multiple values {}".format(best_points))
        return None
